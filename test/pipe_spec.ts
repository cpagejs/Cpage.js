...
import parse from "../packages/parse/index";
import Pipe from "../packages/pipe/pipe";
const p = new Pipe();
...
describe("pipe测试", function () {
  ...
  describe("3)默认注册的array", function () {
    ...
    it("5,过滤对象", function () {
      ...
      expect(fn({ arr: [{ name: "jone" }, { name: "ke" }] })).to.deep.eq([
        { name: "jone" },
      ]);
    });
    ...
    it("6,过滤null", function () {
      ...
      expect(fn({ arr: [{ name: "null" }, { name: "ke" }] })).to.deep.eq([
        { name: "null" },
      ]);
    });
    ...
    it("7,过滤null", function () {
      var fn = parse.parse("arr|filter:null");
      expect(fn({ arr: [null, { name: "null" }, { name: "ke" }] })).to.deep.eq([
        null,
      ]);
    });
    ...
    it("10,过滤对象", function () {
      ...
      expect(fn({ arr: [{ name: "ao" }, { name: "li" }] })).to.deep.eq([
        { name: "ao" },
      ]);
    });
  });
