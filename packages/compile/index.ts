...
store.service("component", function () {
  this.ensureOneInvokeComponent = (name, arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].name == name) rootName.push(name);
    ...
    if (rootName.length >= 2)
      res = {
        type: "no",
        info:
          "只能有一个根组件，却发现" + rootName.length + "个" + name + "根组件",
      };
    return res;
  };
});
...
export default class CPage {
  ...
  static bootstrap(selector, fn) {
    ...
    function componetList(fn, isRoot = false) {
      ...
      store.data(
        "componentList",
        store.get("componentList").push(componentJson)
      );

      if (
        componentJson.components &&
        Util.type(componentJson.components) == "array" &&
    ...
    const r = new renderComponents(
      selector,
      rootComponent,
      store.get("componentList")
    );
    r.componentToDom();
  }
