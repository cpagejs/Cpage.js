...
export default class renderComponents {
  ...
  private cForTpl: object; //c-for 模板集合
  ...
  private loopComponents(components: Array<any>, componentArr: Array<any>, componentName: string, fatherData?: object,) {
    ...
    components.forEach(async (v) => {
      ...
      async function handelCC() {
        await self.handelBeforeRender(v);  //在组建渲染之前执行
        await self.handelAfterRender(v);  //在组件渲染之后执行
   ...
   * @param v 组件
   */
  private async handelAfterRender(v) {
    let self = this;
  ...
  private loopCforToDom(arr, component, reRender?) {
    ...
    currentRepeat.forEach(re => {
      ...
      if (items && Util.type(items) === 'array') {
        // 组件的属性更新了
        if (reRender === 'dataChange') {
          if(DOM.q(re.ele)) {
            DOM.delDomExpectWhich('[c-for-id="' + DOM.q(re.ele).getAttribute('c-for-id') + '"]', 0);
          }else { // 已经被移除，还原节点
            DOM.replaceComment(
              DOM.q(Util._cameCase(re.which)),
              'c-for:'+ re.id,
              DOM.create(this.cForTpl[re.id])[0]
            );
          }
        }
        ...
        if(!items.length) {
          const cForDom = DOM.q(re.ele);
          cForDom.parentNode.replaceChild(DOM.addComment('c-for:' + re.id + ''), cForDom);
